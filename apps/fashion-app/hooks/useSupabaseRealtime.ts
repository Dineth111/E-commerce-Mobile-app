...
export function useSupabaseRealtime() {
  ...
  useEffect(() => {
    ...
    // ─── Subscribe to Review Status Updates for THIS user ───────────────
    reviewsSub = supabase
      .channel(`reviews-user-${user.id}-${Math.random().toString(36).substring(7)}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
        ...
        (payload) => {
          ...
          const oldReview = payload.old as any;

          if (oldReview.status !== newReview.status) {
            if (newReview.status === 'approved') {
              addNotification({
                title: 'Review Approved! 🌟',
                body: 'Your review has been approved and published.',
                type: 'product',
                data: { productId: newReview.product_id },
              });
            } else if (newReview.status === 'rejected') {
              addNotification({
                title: 'Review Rejected ⚠️',
                body: 'Your review was not approved as it did not meet our guidelines.',
                type: 'product',
                data: { productId: newReview.product_id },
              });
            }
